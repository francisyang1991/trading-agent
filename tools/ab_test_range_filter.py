#!/usr/bin/env python3
"""
A/B test for range-bound (mean-reversion) filter.
"""
...
def run_variant(symbols, period, config, args, label):
    ...
    return {
        "variant": label,
        "reject_range_bound": config.reject_range_bound,
        "patterns": total,
...
def main():
    parser = argparse.ArgumentParser(description="Range-bound filter A/B test")
    ...
    parser.add_argument("--export", type=str, default="results/ab_test_range_filter.csv")
    parser.add_argument("--report-md", type=str, default="results/ab_test_range_filter.md")
    ...
    for reject in [False, True]:
        config = VolumePullbackConfig(
            ...
            reject_range_bound=reject,
        )
        label = "range_filter_on" if reject else "range_filter_off"
        variants.append(run_variant(symbols, args.period, config, args, label))
    ...
    print(f"✅ Exported range filter A/B to: {args.export}")
