...
=============================
Deep-dive into company fundamentals beyond basic P/E.

Features:
1. ROIC (Return on Invested Capital) - True profitability
2. FCF Yield - Cash generation ability
3. Piotroski F-Score (0-9) - Value investing score
4. Altman Z-Score - Bankruptcy risk
5. Quality, Value, Growth, Health composite scores

Usage:
    # Analyze single stock
    python tools/fundamental_analyzer.py NVDA
    
    # Compare multiple stocks
    python tools/fundamental_analyzer.py NVDA AMD INTC AVGO
    
    # Focus on specific metrics
    python tools/fundamental_analyzer.py AAPL --focus quality
    
    # Output to file
    python tools/fundamental_analyzer.py GOOGL MSFT META -o tech_comparison.txt
"""
...
from src.universe.fundamentals import (
    FundamentalAnalyzer,
    FundamentalMetrics,
...
def print_comparison_table(metrics_list: List[FundamentalMetrics], file=None):
    ...
    print(f"\n{'='*130}", file=file)
    print(f"  FUNDAMENTAL COMPARISON", file=file)
    ...
    print(f"\n{'Symbol':<8} {'Grade':>6} {'Score':>6} {'Quality':>8} {'Value':>6} {'Growth':>7} {'Health':>7} "
          f"{'F-Score':>8} {'ROIC':>8} {'FCF%':>8} {'P/E':>8} {'D/E':>6}", file=file)
    print("-" * 130, file=file)
    ...
    for m in metrics_list:
        ...
        print(f"{m.symbol:<8} {m.fundamental_grade:>6} {m.overall_fundamental_score:>5.0f} "
              f"{m.quality_score:>7.0f} {m.value_score:>6.0f} {m.growth_score:>6.0f} "
              f"{m.financial_health_score:>6.0f} {m.piotroski_f_score:>7}/9 "
              f"{roic:>8} {fcf:>8} {pe:>8} {de:>6}", file=file)
    
    # Legend
    print(f"\n  SCORING GUIDE:", file=file)
    print(f"    Grade A = 80+, B = 70-79, C = 60-69, D = 50-59, F = <50", file=file)
    print(f"    ROIC: >15% excellent, >10% good | FCF Yield: >5% good", file=file)
    print(f"    Piotroski: 7-9 strong, 4-6 average, 0-3 weak", file=file)
...
def print_detailed_comparison(metrics_list: List[FundamentalMetrics], file=None):
    ...
    print(f"\n{'='*100}", file=file)
    print(f"  DETAILED ANALYSIS", file=file)
    ...
    for m in sorted(metrics_list, key=lambda x: x.overall_fundamental_score, reverse=True):
        ...
        # Scores breakdown
        print(f"\n  Scores:", file=file)
        print(f"    Quality: {m.quality_score:.0f} | Value: {m.value_score:.0f} | "
              f"Growth: {m.growth_score:.0f} | Health: {m.financial_health_score:.0f}", file=file)
...
def print_ranking_by_metric(metrics_list: List[FundamentalMetrics], metric: str, file=None):
    ...
    # Sort (higher is better for most metrics)
...
def main():
    ...
    parser.add_argument("--focus", choices=['quality', 'value', 'growth', 'health', 'all'],
                       default='all', help="Focus on specific aspect")
    ...
    # Print detailed if not brief
    if not args.brief:
        print_detailed_comparison(metrics_list, file=file)
    
    # Rank by specific metric if requested
    if args.rank_by:
        print_ranking_by_metric(metrics_list, args.rank_by, file=file)
    
    # Print individual detailed reports if single stock or focus requested
    if len(args.symbols) == 1 or args.focus != 'all':
        for m in metrics_list:
            if output_file:
                ...
                output_file.write(sys.stdout.getvalue())
